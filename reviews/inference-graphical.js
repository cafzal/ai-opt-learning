...
(window.QUIZ_REVIEWS = window.QUIZ_REVIEWS || {})["inference-graphical"] = {
  ...
  concepts: [
    ...
    {
      title: "Causality: intervention & counterfactuals",
      tag: "graphical",
      body: "<p>Pearl's <b>ladder of causation</b> has three rungs of increasing power, and a Bayes net read passively answers only the first:</p><ul><li><b>(1) Association — seeing.</b> $p(y\\mid x)$: how $y$ varies across units that <i>happen</i> to have a given $x$. This is all conditioning gives you.</li><li><b>(2) Intervention — doing.</b> $p(y\\mid \\mathrm{do}(x))$: the distribution of $y$ if we reach in and <i>force</i> $X=x$ for everyone. Setting $X$ <b>cuts every incoming edge</b> into $X$ (it no longer listens to its parents), so the result differs from $p(y\\mid x)$ whenever a <b>confounder</b> feeds both $X$ and $Y$.</li><li><b>(3) Counterfactuals — imagining.</b> $p(y_{x'}\\mid x,y)$: for <i>this</i> unit that took action $x$ and saw $y$, what <i>would</i> its outcome have been under a different action $x'$? A unit-level 'what if' the data never shows directly.</li></ul><p>Conditioning $p(y\\mid x)$ and intervening $p(y\\mid \\mathrm{do}(x))$ coincide only when there is no open <b>backdoor path</b> — a non-causal path $X\\!\\leftarrow\\!\\dots\\!\\to\\!Y$ that leaks association. The <b>backdoor criterion</b> says: a set $\\mathbf{Z}$ identifies the causal effect if it <i>blocks every backdoor path</i> yet contains <b>no descendant of $X$</b> (adjusting for a descendant or collider can open new spurious paths). Then <b>adjustment</b> recovers it: $p(y\\mid \\mathrm{do}(x))=\\sum_{\\mathbf{z}} p(y\\mid x,\\mathbf{z})\\,p(\\mathbf{z})$. A <b>randomized experiment</b> assigns $X$ by a coin flip, which severs its incoming edges by construction — so confounders cannot enter and the experiment estimates $p(y\\mid \\mathrm{do}(x))$ directly, no adjustment needed.</p>",
      example: "A drug is prescribed more to sicker patients (severity → both treatment and death). Observationally $p(\\text{death}\\mid\\text{treated})$ can exceed $p(\\text{death}\\mid\\text{untreated})$ — the drug looks harmful. Adjusting for severity, $\\sum_s p(\\text{death}\\mid\\text{treated},s)\\,p(s)$, gives $p(\\text{death}\\mid \\mathrm{do}(\\text{treat}))$ and can flip the sign (Simpson's paradox is this confounding made visible). A randomized trial would have estimated the $\\mathrm{do}$-effect with no adjustment at all.",
      takeaway: "Prediction lives on rung 1, but decisions and 'what if' questions need rungs 2–3: identify a valid adjustment set with the backdoor criterion — or randomize — before reading any number as a causal effect."
    },
    {
