...
(window.QUIZ_REVIEWS = window.QUIZ_REVIEWS || {})["kernels-trees"] = {
  ...
  concepts: [
    ...
    },
    {
      ...
      tag: "ensembles",
      body: "<p><b>Bagging</b> trains $M$ models on bootstrap resamples and averages them. It <b>cuts variance</b> (bias unchanged) for <i>unstable</i> learners like trees; the ~37% of points left out of each bootstrap give a free <b>out-of-bag</b> error estimate. Ensemble variance $\\to\\rho\\sigma^2$ as $M{\\to}\\infty$, so <b>decorrelation</b> is what helps. <b>Random forests</b> add a random feature subset ($\\sim\\!\\sqrt{D}$) at each split to decorrelate further — a gold-standard tabular baseline.</p><p><b>Boosting</b> is sequential: an additive model $f=\\sum_m\\beta_m F_m$ where each weak learner targets the <i>current errors</i>, so it <b>cuts bias</b>. <b>AdaBoost</b> (exponential loss) up-weights misclassified points with $\\beta_m=\\tfrac12\\log\\tfrac{1-\\text{err}_m}{\\text{err}_m}$; <b>gradient boosting / MART</b> does functional gradient descent, fitting each tree to the negative-gradient <b>pseudo-residuals</b> (squared loss → residuals; log loss → calibrated LogitBoost). XGBoost / LightGBM / CatBoost add second-order info, tree regularization, and subsampling.</p><p><b>Bagging vs boosting</b> — same building block, opposite philosophies: bagging is <b>parallel/independent</b>, averages or votes, rarely overfits, and is <b>robust to outliers</b>; boosting is <b>sequential/dependent</b>, takes a weighted sum, <b>can overfit</b> noisy labels, and (AdaBoost especially) is <b>outlier-sensitive</b>.</p>",
      example: "A single deep tree overfits. Bag 500 of them with $\\sqrt{D}$-feature splits → a random forest that just works out of the box (robust, parallel, OOB diagnostics). Or gradient-boost shallow trees (depth 1–5) with shrinkage $\\nu\\approx0.05$–$0.1$ and subsampling, each fitting the running pseudo-residuals → usually the most accurate model on tabular data, at the cost of being sequential and tunable-into-overfitting.",
      takeaway: "Ensembles are why trees dominate tabular ML, and the everyday fork is this: default to a random forest for a fast robust parallel baseline, then switch to gradient boosting only when the last accuracy points justify the tuning budget."
    },
    {
