...
(window.QUIZ_REVIEWS = window.QUIZ_REVIEWS || {})["constrained-opt"] = {
  ...
  concepts: [
    ...
    {
      title: "Sensitivity under degeneracy: one-sided prices, reduced costs & conflicts",
      tag: "duality",
      body: "<p>The clean reading “multiplier = shadow price = $\\partial p^\\star/\\partial b$” quietly assumes the optimal dual is <b>unique</b>. Degeneracy breaks that assumption in two directions — exactly where solver sensitivity reports mislead.</p><p><b>Reduced costs.</b> At an LP optimum, each nonbasic (zero) variable carries a <b>reduced cost</b> $r_j=c_j-\\boldsymbol\\lambda^\\top\\mathbf{a}_j\\ge 0$: how much its objective coefficient must improve before including it would pay — the entry price of an excluded activity. A reduced cost of <b>exactly zero on a nonbasic variable</b> is <b>dual degeneracy</b>: the optimal <i>value</i> is flat along that direction, so <b>alternative optima</b> exist — the returned plan is one of several equally good ones, and a tiny data change can swap which one the solver reports.</p><p><b>Primal degeneracy ⇒ ambiguous shadow prices.</b> At a degenerate vertex (more than $n$ active constraints) the optimal <i>duals</i> are not unique: the value function $p^\\star(b)$ has a <b>kink</b> at the current right-hand side, and the set of optimal duals spans its left and right slopes. Three practical consequences: (i) a shadow price is valid <b>one-sided</b> — the value of one <i>more</i> unit of a resource and the cost of one <i>less</i> genuinely differ (a buy price and a sell price at the kink); (ii) different solvers or pivot rules can report <b>different duals for the same model</b>, all “correct”; (iii) the report's <b>allowable increase/decrease can be zero-width</b> — the basis changes immediately even where the price persists. The robust move: treat reported duals as <i>directional</i> — re-solve with the right-hand side nudged each way and read the two slopes off the actual value change. In practice this is a loop: duals give the <b>direction</b> (which lever to examine first), a cheap re-solve gives the <b>price</b> of the move, and scenario re-optimization (see <i>Optimization under Uncertainty</i>) stress-tests the resulting plan across whole futures.</p><p><b>Conflict analysis.</b> Duality also speaks when there is <i>no</i> solution. If $\\mathbf{A}\\mathbf{x}\\le\\mathbf{b}$ is infeasible, there exist weights $\\mathbf{y}\\ge\\mathbf{0}$ with $\\mathbf{y}^\\top\\mathbf{A}=\\mathbf{0}^\\top$ and $\\mathbf{y}^\\top\\mathbf{b}<0$ — a <b>Farkas certificate</b> (dual ray): the $y$-weighted sum of the constraints reads $0\\le\\mathbf{y}^\\top\\mathbf{b}<0$, an explicit contradiction. The positively-weighted rows name the conflict; solvers refine it to an <b>IIS</b> (irreducible infeasible subsystem) — a minimal set that cannot coexist, every member necessary. And note the plural: a model can hide <i>several</i> conflicts, so repair the reported one and re-check.</p>",
      visual: `<svg viewBox="0 0 520 240" xmlns="http://www.w3.org/2000/svg" role="img">
        <text x="10" y="20" style="fill:var(--text)" font-size="13" font-weight="700">Degenerate optimum: the value function kinks</text>
        <line x1="60" y1="195" x2="495" y2="195" class="vx-axis" stroke-width="1.5"/>
        <line x1="60" y1="195" x2="60" y2="30" class="vx-axis" stroke-width="1.5"/>
        <text x="278" y="224" text-anchor="middle" font-size="12" style="fill:var(--text-dim)">right-hand side b (resource) →</text>
        <text x="56" y="34" font-size="11" style="fill:var(--text-faint)" text-anchor="end">p*(b)</text>
        <line x1="270" y1="118" x2="270" y2="195" class="vx-grid" stroke-width="1.2" stroke-dasharray="4 4"/>
        <line x1="270" y1="118" x2="460" y2="200" class="vx-warn" stroke-width="1.3" stroke-dasharray="5 4"/>
        <line x1="270" y1="118" x2="460" y2="176" class="vx-warn" stroke-width="1.3" stroke-dasharray="2 4"/>
        <line x1="85" y1="38" x2="270" y2="118" class="vx-bad" stroke-width="2.5"/>
        <line x1="270" y1="118" x2="460" y2="152" class="vx-good" stroke-width="2.5"/>
        <circle cx="270" cy="118" r="5" style="fill:var(--warn)"/>
        <text x="270" y="212" text-anchor="middle" font-size="11" style="fill:var(--text-dim)">current b</text>
        <text x="110" y="44" font-size="11" style="fill:var(--bad)">one unit less: costs λ⁻</text>
        <text x="300" y="104" font-size="11" style="fill:var(--good)">one unit more: worth only λ⁺</text>
        <text x="455" y="190" text-anchor="end" font-size="10.5" style="fill:var(--warn)">reported duals span these slopes</text>
      </svg>`,
      caption: "At a degenerate vertex p*(b) is kinked: the left and right slopes differ, and the set of optimal duals spans them — so “the” shadow price depends on direction.",
      example: "A plant's optimum has machine capacity, a delivery contract, <i>and</i> a materials limit all binding at the same corner — degenerate. One solver prints capacity's shadow price as \\$0 (an extra hour is worthless; materials still cap output), another prints \\$500 (losing an hour breaches the contract). Both are right — one-sided. And when sales push the contract above capacity, the Farkas/IIS output names exactly the pair {contract, capacity} to renegotiate: a map of the conflict, not just “infeasible.”",
      takeaway: "Before acting on a shadow price, check for degeneracy: prices can be direction-dependent, ranges vacuous, and two solvers can disagree without either being wrong — nudge the RHS both ways and re-solve for the true marginal values. When the model is infeasible, ask for the certificate/IIS: it tells you which requirements conflict and what to relax."
    },
    {
