(window.QUIZ_REVIEWS = window.QUIZ_REVIEWS || {})["stage0"] = {
  ...
  concepts: [
    ...
    {
      title: "Prediction is not intervention",
      tag: "causality",
      body: "<p>A model trained on <b>observational</b> data answers one question: what am I likely to see if the world keeps running the way it did when the data was collected? A <b>decision</b> asks a different one — it is an <i>intervention</i> that changes the data-generating process. Conditioning is not intervening: $p(y\\mid x)$ describes units that <i>happened</i> to have $x$, while acting requires $p(y\\mid \\mathrm{do}(x))$ — what follows if you <i>force</i> $x$ — and the two differ whenever a <b>confounder</b> drives both $x$ and the outcome.</p>" +
        "<p>There's a second twist: acting on the model's predictions <i>shifts the future data</i>. Once forecasts drive interventions, the next dataset is generated by a different process than the one the model learned — a feedback loop that is the proxy gap's dynamic cousin: optimize against a learned pattern and the pattern moves. The machinery for getting this right — the do-operator, backdoor adjustment, randomized experiments — lives in Stage 3.</p>",
      example: "A churn model learns which usage patterns predict cancellation, so the business offers discounts to everyone it flags. The discounts change churn behavior (that was the point) — the association the model learned no longer holds under the intervention — and next quarter's training data now mixes true risk with the effect of the offers, quietly poisoning the retrained model.",
      takeaway: "A predictive model tells you what you'll see if you keep watching; a decision changes what there is to see. Before reading $p(y\\mid x)$ as \"what happens if I act,\" demand an experiment or a causal adjustment — Stage 3 builds exactly that machinery."
    },
    {
