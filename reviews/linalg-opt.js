...
,
    ...
    },
    {
      ...
      tag: "core",
      body: "<p><b>Eigendecomposition:</b> $\\mathbf{A}\\boldsymbol{v}=\\lambda\\boldsymbol{v}$. For a <b>symmetric</b> matrix the spectral theorem gives $\\mathbf{A}=\\mathbf{U}\\boldsymbol\\Lambda\\mathbf{U}^\\top=\\sum_i\\lambda_i\\boldsymbol{u}_i\\boldsymbol{u}_i^\\top$ with $\\mathbf{U}$ orthogonal; the level sets of $\\boldsymbol{x}^\\top\\mathbf{A}\\boldsymbol{x}$ are ellipsoids whose axes are the eigenvectors. <b>Whitening</b> $\\mathbf{W}=\\boldsymbol\\Lambda^{-1/2}\\mathbf{U}^\\top$ maps data to identity covariance.</p><p><b>Repeated (degenerate) eigenvalues.</b> When an eigenvalue $\\lambda$ repeats, its eigenvectors span an entire <b>eigenspace</b>, not a single line — and within that space <i>no individual eigenvector is unique</i> (any orthonormal basis of it will do, a rotational degeneracy). Two multiplicities measure this: the <b>algebraic multiplicity</b> is how many times $\\lambda$ is a root of $\\det(\\mathbf{A}-\\lambda\\mathbf{I})=0$; the <b>geometric multiplicity</b> is $\\dim\\ker(\\mathbf{A}-\\lambda\\mathbf{I})$, the eigenspace's dimension. Always geometric $\\le$ algebraic. When geometric $<$ algebraic there aren't enough independent eigenvectors to form a basis, so the matrix is <b>defective</b> — <i>not diagonalizable</i> (you must fall back on the Jordan form). Symmetric matrices are never defective: the spectral theorem forces geometric $=$ algebraic, so a repeated $\\lambda$ just means an isotropic (round) subspace with a free choice of axes there.</p><p><b>SVD</b> generalizes to any $m\\times n$ matrix: $\\mathbf{A}=\\mathbf{U}\\boldsymbol\\Sigma\\mathbf{V}^\\top$, where the right singular vectors are the eigenvectors of $\\mathbf{A}^\\top\\mathbf{A}$ with $\\sigma_i^2=\\lambda_i$. The <b>pseudo-inverse</b> $\\mathbf{A}^+=\\mathbf{V}\\boldsymbol\\Sigma^+\\mathbf{U}^\\top$ (invert each $\\sigma_i$) gives the least-squares solution $\\hat{\\boldsymbol{x}}=\\mathbf{A}^+\\boldsymbol{b}$. <b>Eckart–Young:</b> keeping the top $K$ singular triples is the best rank-$K$ approximation, with squared error $\\sum_{k>K}\\sigma_k^2$ — this powers PCA, LSA, recommenders, and compression.</p>",
