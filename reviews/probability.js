  ...
  concepts: [
    {
      ...
      tag: "core",
      body: "<p>The <b>frequentist</b> view says probability is the long-run frequency of a repeatable event; the <b>Bayesian</b> view says it is a degree of belief about <i>any</i> uncertain proposition. The axioms are identical — ML leans Bayesian because it must assign probabilities to parameters and one-off events ('is <i>this</i> email spam?'), which have no repeatable frequency.</p><p>Uncertainty splits two ways:</p><ul><li><b>Aleatoric</b> — irreducible randomness in the process (a fair coin). It lives in the <i>likelihood</i> and cannot be reduced with more data.</li><li><b>Epistemic</b> — reducible uncertainty from lack of data/knowledge. It is tracked by the <i>posterior</i> and shrinks as data arrives.</li></ul><p>A sharper, decision-theoretic refinement grades <i>how much you know about the probabilities themselves</i> — the <b>risk / uncertainty / ambiguity</b> ladder (Knight, 1921):</p><ul><li><b>Risk</b> — the distribution is <i>known</i> (a fair die: $p=1/6$). You can compute expected loss directly.</li><li><b>(Statistical) uncertainty</b> — the distribution's <i>form</i> is known but its parameters are <i>estimated from data</i> (a coin of unknown bias $\\theta$). This is epistemic uncertainty over parameters — it shrinks with more data.</li><li><b>Ambiguity / Knightian uncertainty</b> — the <i>distribution itself is unknown</i> (you don't even know the family, or whether tomorrow resembles the past). No amount of in-distribution data pins it down.</li></ul><p>Ambiguity is precisely what motivates <b>robust / minimax / distributionally-robust</b> methods (Stage 4): when you can't trust a single $p$, optimize against the worst case over a whole <i>set</i> of plausible distributions instead of a point estimate.</p>",
      ...
      takeaway: "Splitting the two tells you whether to collect more data (epistemic shrinks) or accept the noise floor (aleatoric won't budge) — and the risk/uncertainty/ambiguity ladder adds a third option: when you can't even trust the distribution, go robust rather than collect more in-distribution data."
    },
    {
